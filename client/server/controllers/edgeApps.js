...
const getEdgeAppList = async (req, res, next) => {
  try {
    ...
    get(res, url)
      ...
      .then(appList => {
        const resAppList = appList.data,
          tags = []
        let loopDataCount = 0
        resAppList.list.forEach(async item => {
          try {
            let tagsData = await get(
              res,
              routes.edgeApp.deviceStatus.replace('{id}', item.deviceId)
            )
            await delay(800)
            if (
              tagsData &&
              ...
              if (
                ipArray.length === 2 &&
                !tags.some(item => item.ipAddrs === ipArray[0])
              ) {
                tags.push({
                  isTagInfo: true,
                  name: 'opcua_server',
            ...
            if (ipCheck.data.kind === networkStatus.local) {
              if (ipCheck.data.assignedAdapters.length > 0) {
                if (!nodeInfo)
                  nodeInfo = await get(
                    res,
                    routes.edgeApp.endgeInstanceInfo.replace('{id}', id)
                  )
                if (nodeInfo.data) {
                  IPInfo = (nodeInfo.data.netStatusList || []).find(
                    data => data.ifName === item.intfname
                  )
                  if (IPInfo)
                    IPInfo = {
                      ...IPInfo,
                      isMemberActive:
                        IPInfo.ifName === ipCheck.data.assignedAdapters[0].name,
                    }
                }
              }
