...
const doLogin = async (req, res, next) => {
  try {
    ...
    if (!!(payload.username && payload.password)) {
      const optionReq = await fetchOptions(
        cluster,
        routes.login,
        'options',
        loginPayloadOptimize({
      ...
      const loginPost = await post(
        res,
        `${cluster}version${routes.login}`,
        loginPayloadOptimize({
          usernameAtRealm: payload.username,
          password: payload.password,
        }),
    ...
    formatResponse(
      ...
      e?.response?.data || {},
      'could not contact cluster URL. Please try again later.'
    )
  }
}
...
const doLoginWithToken = async (req, res, next) => {
  try {
    ...
    if (payload.token) {
      ...
      const loginGet = await get(
        res,
        `${cluster}version${routes.loginWithToken}`
      )
      ...
      if (!data) {
        formatResponse(
          ...
          400,
          data,
          'could not contact cluster URL, please reach out to SysAdmin'
        )
      } else {
        const userResponse = {}
        ...
        res.status(200).send({
          ...
          statusCode: 200,
          data: userResponse,
          message: 'Token login in succesfully!',
        })
      }
    } else {
      formatResponse(res, 400, null, 'Invalid username or password')
...
const updateTerm = async (req, res, next) => {
  ...
  try {
    if (userID) {
      const updateRequest = User.updateUserItem(
        userID,
        agreeStatus,
        moment().format()
      )
      ...
      return formatResponse(
        ...
        400,
        { status: null },
        'User id not exist to update Terms of service!'
      )
    }
  } catch (e) {
    formatResponse(
      ...
      e.response.data.httpStatusCode || 400,
      e.response.data || {},
      'Terms of service update failed!'
    )
  }
}
